...
cursor = conn.cursor()
cursor.execute('SELECT * FROM snake_game LIMIT 10')
...
if add_:
    postgres_insert_query = """ INSERT INTO snake_game (name_,score,total_score) VALUES (%s,%s,%s)"""
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN :# полностью движение 
            ...
            if event.key == pygame.K_q:
                sql_update_query = """Update snake_game set score = %s where name_ = %s"""
...
total_score+=count
sql_update_query = """Update snake_game set total_score= %s where name_ = %s"""
